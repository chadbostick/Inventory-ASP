...
<title>Centerline - Customer</title>
  ...
  function OpenCustomer( strCustomerId )
	{
	  OpenWindow( 'customer_details.asp?intCustomerId='+strCustomerId,'Centerline','width=657,height=550,left='+GetCenterX( 657 )+',top='+GetCenterY( 550 )+',screenX='+GetCenterX( 657 )+',screenY='+GetCenterY( 550 )+',scrollbars=yes,dependent=yes' );
	}
	...
	function JumpToSubmit()
	{
		...
		{
			document.location.href = "admin_customer.asp?blnIsJumpTo=1&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if ( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "admin_customer.asp?blnIsJumpTo=1&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	...
	function SearchSubmit()
	{
		if( document.all('strSearchTop').value != "" )
		{
			document.location.href = "admin_customer.asp?blnIsJumpTo=0&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "admin_customer.asp?blnIsJumpTo=0&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
		
	function ConfirmDelete( intCustomerId, strCustomer )
	{
		if( confirm( 'Are you sure you want to remove ' + strCustomer + '?' ) )
		{
			OpenWindow( 'customer_delete.asp?intCustomerId='+intCustomerId,'DeleteRecord','width=657,height=120,left='+GetCenterX( 657 )+',top='+GetCenterY( 120 )+',screenX='+GetCenterX( 657 )+',screenY='+GetCenterY( 120 )+',scrollbars=yes,dependent=yes' );
		}
	}
   ...
   <td><table border=0 cellpadding=0 cellspacing=0 width=374>
     ...
     <tr><td rowspan=2><IMG SRC="images/menu/menu_bottom_search_left.gif" width=8 height=31 border=0></td>
       <td width=302><table border=0 cellpadding=0 cellspacing=0 width=366>
         <tr><td valign=bottom width=30><a href="admin_customer.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=27 height=20 border=0></a></td>
				   <td valign=bottom width=27><a href="admin_customer.asp?intRowStart=<%=intPreviousRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/previous_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="admin_customer.asp?intRowStart=<%=intNextRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/next_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=30><a href="admin_customer.asp?intRowStart=<%=intLastRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/last_button.gif" width=27 height=20 border=0></a></td>
...
<tr>
  ...
  <tr>
    ...
    <th class="itemlist" width=212><a href="admin_customer.asp?strOrderBy=CustomerName">Customer</a></th>
    <th class="itemlist" width=120><a href="admin_customer.asp?strOrderBy=Contact">Contact</a></th>
    <th class="itemlist" width=140><a href="admin_customer.asp?strOrderBy=Address">Address</a></th>
    <th class="itemlist" width=90><a href="admin_customer.asp?strOrderBy=City">City</a></th>
    <th class="itemlist" width=90>Phone</th>
  </tr>
  <!-- table data -->
	<%
	if( rsGetCustomers.State = adStateOpen ) then
		while( not rsGetCustomers.EOF )
			...
			<tr class="evenrow">
				<td class="itemlist" width=20 align=middle><a href="javascript:ConfirmDelete('<%=rsGetCustomers( "CustomerId" )%>', '<%=rsGetCustomers( "Customer" )%>');"><img src="images/form_delete.gif" width=15 height=15 border=0></a></td>
				<td class="itemlist" <%if( strOrderBy = "CustomerName" ) then%> id="selected_evenrow"<%end if%>><a href="javascript:OpenCustomer( '<%=rsGetCustomers( "CustomerId" )%>' );"><%=rsGetCustomers( "Customer" )%></a></td>
				<td class="itemlist" <%if( strOrderBy = "Contact" ) then%> id="selected_evenrow"<%end if%>><%=rsGetCustomers( "Contact" )%></td>
				<td class="itemlist" <%if( strOrderBy = "Address" ) then%> id="selected_evenrow"<%end if%>><%=rsGetCustomers( "Address" )%></td>
				<td class="itemlist" <%if( strOrderBy = "City" ) then%> id="selected_evenrow"<%end if%>><%=rsGetCustomers( "City" )%></td>
				<td class="itemlist"><%=rsGetCustomers( "TelephoneNumber" )%></td>
			<%else%>
			<tr class="oddrow">
				<td class="itemlist" width=20 align=middle><a href="javascript:ConfirmDelete('<%=rsGetCustomers( "CustomerId" )%>', '<%=rsGetCustomers( "Customer" )%>');"><img src="images/form_delete.gif" width=15 height=15 border=0></a></td>
				<td class="itemlist" <%if( strOrderBy = "CustomerName" ) then%> id="selected_oddrow"<%end if%>><a href="javascript:OpenCustomer( '<%=rsGetCustomers( "CustomerId" )%>' );"><%=rsGetCustomers( "Customer" )%></a></td>
				<td class="itemlist" <%if( strOrderBy = "Contact" ) then%> id="selected_oddrow"<%end if%>><%=rsGetCustomers( "Contact" )%></td>
				<td class="itemlist" <%if( strOrderBy = "Address" ) then%> id="selected_oddrow"<%end if%>><%=rsGetCustomers( "Address" )%></td>
				<td class="itemlist" <%if( strOrderBy = "City" ) then%> id="selected_oddrow"<%end if%>><%=rsGetCustomers( "City" )%></td>
				<td class="itemlist"><%=rsGetCustomers( "TelephoneNumber" )%></td>
			<%end if%>
			</tr>
