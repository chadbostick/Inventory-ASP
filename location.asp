...
dim rsGetLocations, cmdGetLocations
dim lngTotalLocationCount, intRowStart, intRowCount, intNextRecord, intPreviousRecord, intFirstRecord, intLastRecord, strOrderBy, strSearchString, strJumpTo
dim strCurrentPage

set cnnSQLConnection	= nothing
set rsGetLocations		= nothing
set cmdGetLocations		= nothing

set cnnSQLConnection	= Server.CreateObject( "ADODB.Connection" )
set cmdGetLocations		= Server.CreateObject( "ADODB.Command" )

cnnSQLConnection.Open Application( "ODBC_DSN" )
cnnSQLConnection.CursorLocation = adUseClient

cmdGetLocations.ActiveConnection	= cnnSQLConnection

cmdGetLocations.CommandText		= "{call spGetLocations( ?,?,?,?,?,? ) }"

cmdGetLocations.Parameters.Append cmdGetLocations.CreateParameter( "@ErrorMessage", adVarChar, adParamOutput, 255 )
cmdGetLocations.Parameters.Append cmdGetLocations.CreateParameter( "@RowStart", adInteger, adParamInputOutput, 4 )
cmdGetLocations.Parameters.Append cmdGetLocations.CreateParameter( "@RowCount", adInteger, adParamInputOutput, 4 )
cmdGetLocations.Parameters.Append cmdGetLocations.CreateParameter( "@OrderBy", adVarChar, adParamInput, 64 )
cmdGetLocations.Parameters.Append cmdGetLocations.CreateParameter( "@SearchString", adVarChar, adParamInput, 255 )
cmdGetLocations.Parameters.Append cmdGetLocations.CreateParameter( "@JumpTo", adVarChar, adParamInput, 9 )
...
cmdGetLocations.Parameters( "@RowStart" ).Value			= intRowStart
cmdGetLocations.Parameters( "@RowCount" ).Value			= Request.QueryString( "intRowCount" )
cmdGetLocations.Parameters( "@OrderBy" ).Value			= Request.QueryString( "strOrderBy" )
cmdGetLocations.Parameters( "@SearchString" ).Value	= strSearchString
cmdGetLocations.Parameters( "@JumpTo" ).Value				= strJumpTo

on error resume next
set rsGetLocations = cmdGetLocations.Execute
...
intRowCount = cmdGetLocations.Parameters( "@RowCount" ).Value

if( rsGetLocations.State = adStateOpen ) then
		lngTotalLocationCount = rsGetLocations( "TotalRowCount" )
		
		set rsGetLocations = rsGetLocations.NextRecordset
		
		if( rsGetLocations.State = adStateOpen ) then
			if( not rsGetLocations.EOF ) then
				intRowStart = rsGetLocations( "RowNumber" )
			end if
		end if
end if

if( ( intRowStart + intRowCount ) > lngTotalLocationCount ) then
	intNextRecord = 0
else
...
intFirstRecord = 1
intLastRecord = lngTotalLocationCount - intRowCount + 1
...
<html>
<head>
<title>Centerline - Locations</title>
  ...
  function OpenLocation( strLocationId )
	{
	  OpenWindow( 'location_details.asp?intLocationId='+strLocationId,'Centerline','width=640,height=120,left='+GetCenterX( 640 )+',top='+GetCenterY( 160 )+',screenX='+GetCenterX( 160 )+',screenY='+GetCenterY( 480 )+',scrollbars=no,dependent=yes' );
	}
	...
	function JumpToSubmit()
	{
		...
		{
			document.location.href = "location.asp?blnIsJumpTo=1&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if ( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "location.asp?blnIsJumpTo=1&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	...
	function SearchSubmit()
	{
		if( document.all('strSearchTop').value != "" )
		{
			document.location.href = "location.asp?blnIsJumpTo=0&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "location.asp?blnIsJumpTo=0&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	
	function ConfirmDelete( intLocationId, strLocation )
	{
		if( confirm( 'Are you sure you want to remove ' + strLocation + '?' ) )
		{
			OpenWindow( 'location_delete.asp?intLocationId='+intLocationId,'DeleteRecord','width=657,height=120,left='+GetCenterX( 657 )+',top='+GetCenterY( 120 )+',screenX='+GetCenterX( 657 )+',screenY='+GetCenterY( 120 )+',scrollbars=yes,dependent=yes' );
		}
	}
   ...
   <td><table border=0 cellpadding=0 cellspacing=0 width=374>
     ...
     <tr><td rowspan=2><IMG SRC="images/menu/menu_bottom_search_left.gif" width=8 height=31 border=0></td>
       <td width=302><table border=0 cellpadding=0 cellspacing=0 width=366>
         <tr><td valign=bottom width=30><a href="location.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=27 height=20 border=0></a></td>
				   <td valign=bottom width=27><a href="location.asp?intRowStart=<%=intPreviousRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/previous_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="location.asp?intRowStart=<%=intNextRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/next_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=30><a href="location.asp?intRowStart=<%=intLastRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/last_button.gif" width=27 height=20 border=0></a></td>
...
<tr>
  ...
  <tr>
    ...
    <th class="itemlist"><a href="location.asp?strOrderBy=Location">Location</a></th>
  </tr>  
  <!-- table data -->
  <%
	if( rsGetLocations.State = adStateOpen ) then
		while( not rsGetLocations.EOF )
			if( ( CInt( rsGetLocations( "RowNumber" ) ) Mod 2 ) = 1 ) then%>
			<tr class="evenrow">
			<%else%>
			<tr class="oddrow">
			<%end if%>
				<td class="itemlist" width=20 align=middle><a href="javascript:ConfirmDelete('<%=rsGetLocations( "LocationId" )%>', '<%=rsGetLocations( "Location" )%>');"><img src="images/form_delete.gif" width=15 height=15 border=0></a></td>
				<td class="itemlist"><a href="javascript:OpenLocation( '<%=rsGetLocations( "LocationId" )%>' );"><%=rsGetLocations( "Location" )%></a></td>
			</tr>
			<%rsGetLocations.MoveNext
		wend
	end if
	%>
</table><!-- End of Locations --></td></tr>
...
<tr><td><table cellspacing=0 cellpadding=0 width=672 align=right>
  ...
  <tr><td valign=bottom width=30><a href="location.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=27 height=20 border=0></a></td>
    ...
    <td align=right valign=bottom width=236><a href="javascript:OpenLocation( 0 );"><IMG src="images/add_button.gif" width=60 height=20 border=0></a></td>
...
<%
if not( rsGetLocations is nothing ) then
	if( rsGetLocations.State = adStateOpen ) then
		rsGetLocations.Close
	end if
end if
