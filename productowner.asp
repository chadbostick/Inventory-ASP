...
dim lngTotalProductOwnerCount, intRowStart, intRowCount, intNextRecord, intPreviousRecord, intFirstRecord, intLastRecord, strOrderBy, strSearchString, strJumpTo
dim strCurrentPage
...
cmdGetProductOwners.CommandText				= "{call spGetProductOwners( ?,?,?,?,?,? ) }"

cmdGetProductOwners.Parameters.Append cmdGetProductOwners.CreateParameter( "@ErrorMessage", adVarChar, adParamOutput, 255 )
cmdGetProductOwners.Parameters.Append cmdGetProductOwners.CreateParameter( "@RowStart", adInteger, adParamInputOutput, 4 )
cmdGetProductOwners.Parameters.Append cmdGetProductOwners.CreateParameter( "@RowCount", adInteger, adParamInputOutput, 4 )
cmdGetProductOwners.Parameters.Append cmdGetProductOwners.CreateParameter( "@OrderBy", adVarChar, adParamInput, 64 )
cmdGetProductOwners.Parameters.Append cmdGetProductOwners.CreateParameter( "@SearchString", adVarChar, adParamInput, 255 )
cmdGetProductOwners.Parameters.Append cmdGetProductOwners.CreateParameter( "@JumpTo", adVarChar, adParamInput, 9 )
...
cmdGetProductOwners.Parameters( "@RowStart" ).Value			= intRowStart
cmdGetProductOwners.Parameters( "@RowCount" ).Value			= Request.QueryString( "intRowCount" )
cmdGetProductOwners.Parameters( "@OrderBy" ).Value			= Request.QueryString( "strOrderBy" )
cmdGetProductOwners.Parameters( "@SearchString" ).Value	= strSearchString
cmdGetProductOwners.Parameters( "@JumpTo" ).Value				= strJumpTo
...
intRowCount = cmdGetProductOwners.Parameters( "@RowCount" ).Value

if( rsGetProductOwners.State = adStateOpen ) then
		lngTotalProductOwnerCount = rsGetProductOwners( "TotalRowCount" )
		
		set rsGetProductOwners = rsGetProductOwners.NextRecordset
		
		if( rsGetProductOwners.State = adStateOpen ) then
			if( not rsGetProductOwners.EOF ) then
				intRowStart = rsGetProductOwners( "RowNumber" )
			end if
		end if
end if

if( ( intRowStart + intRowCount ) > lngTotalProductOwnerCount ) then
	intNextRecord = 0
else
...
intFirstRecord = 1
intLastRecord = lngTotalProductOwnerCount - intRowCount + 1
...
<title>Centerline - Sub Work Sources</title>
  ...
  function OpenProductOwner( strProductOwnerId )
	{
	  OpenWindow( 'ProductOwner_details.asp?intProductOwnerId='+strProductOwnerId,'Centerline','width=640,height=120,left='+GetCenterX( 640 )+',top='+GetCenterY( 160 )+',screenX='+GetCenterX( 160 )+',screenY='+GetCenterY( 480 )+',scrollbars=no,dependent=yes' );
	}
	...
	function JumpToSubmit()
	{
		...
		{
			document.location.href = "ProductOwner.asp?blnIsJumpTo=1&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if ( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "ProductOwner.asp?blnIsJumpTo=1&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	...
	function SearchSubmit()
	{
		if( document.all('strSearchTop').value != "" )
		{
			document.location.href = "ProductOwner.asp?blnIsJumpTo=0&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "ProductOwner.asp?blnIsJumpTo=0&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	
	function ConfirmDelete( intProductOwnerId, strProductOwner )
	{
		if( confirm( 'Are you sure you want to remove ' + strProductOwner + '?' ) )
		{
			OpenWindow( 'ProductOwner_delete.asp?intProductOwnerId='+intProductOwnerId,'DeleteRecord','width=657,height=120,left='+GetCenterX( 657 )+',top='+GetCenterY( 120 )+',screenX='+GetCenterX( 657 )+',screenY='+GetCenterY( 120 )+',scrollbars=yes,dependent=yes' );
		}
	}
	...
	function ProductOwnerSaved( intId, strName )
	{
		RefreshCurrentPage();
	}
  </script>
   ...
   <td><table border=0 cellpadding=0 cellspacing=0 width=374>
     ...
     <tr><td rowspan=2><IMG SRC="images/menu/menu_bottom_search_left.gif" width=8 height=31 border=0></td>
       <td width=302><table border=0 cellpadding=0 cellspacing=0 width=366>
         <tr><td valign=bottom width=30><a href="ProductOwner.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=27 height=20 border=0></a></td>
				   <td valign=bottom width=27><a href="ProductOwner.asp?intRowStart=<%=intPreviousRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/previous_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="ProductOwner.asp?intRowStart=<%=intNextRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/next_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=30><a href="ProductOwner.asp?intRowStart=<%=intLastRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/last_button.gif" width=27 height=20 border=0></a></td>
...
<tr>
  ...
  <tr>
    ...
    <th class="itemlist"><a href="ProductOwner.asp?strOrderBy=ProductOwner">Product Owner</a></th>
  </tr>  
  <!-- table data -->
	<%if( rsGetProductOwners.State = adStateOpen ) then
			while( not rsGetProductOwners.EOF )
				if( ( CInt( rsGetProductOwners( "RowNumber" ) ) Mod 2 ) = 1 ) then%>
					<tr class="evenrow">
				<%else%>
					<tr class="oddrow">
				<%end if%>
						<td class="itemlist" width=20 align=middle><a href="javascript:ConfirmDelete('<%=rsGetProductOwners( "ProductOwnerId" )%>', '<%=rsGetProductOwners( "ProductOwner" )%>');"><img src="images/form_delete.gif" width=15 height=15 border=0></a></td>
						<td class="itemlist"><a href="javascript:OpenProductOwner('<%=rsGetProductOwners( "ProductOwnerId" )%>');"><%=rsGetProductOwners( "ProductOwner" )%></a></td>
				</tr>
...
<tr><td><table cellspacing=0 cellpadding=0 width=672 align=right>
  ...
  <tr><td valign=bottom width=30><a href="ProductOwner.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=27 height=20 border=0></a></td>
    ...
    <td align=right valign=bottom width=236><a href="javascript:OpenProductOwner( 0 );"><IMG src="images/add_button.gif" width=60 height=20 border=0></a></td>
