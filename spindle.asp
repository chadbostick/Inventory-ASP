...
if( ( intRowStart - intRowCount ) < 0 ) then
	intPreviousRecord = lngTotalSpindleCount - intRowCount
else
	intPreviousRecord = intRowStart - intRowCount + 1
end if

intFirstRecord = 1
intLastRecord = lngTotalSpindleCount - intRowCount
...
<title>Centerline - Spindle</title>
  ...
  function OpenSpindle( strSpindleId )
	...
	function JumpToSubmit()
	{
		...
		{
			document.location.href = "spindle.asp?blnIsJumpTo=1&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if ( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "spindle.asp?blnIsJumpTo=1&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	...
	function SearchSubmit()
	{
		if( document.all('strSearchTop').value != "" )
		{
			document.location.href = "spindle.asp?blnIsJumpTo=0&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "spindle.asp?blnIsJumpTo=0&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
...
<tr><td><table border=0 cellpadding=0 cellspacing=0 width=770>
  <tr><td><a href="index.html"><IMG SRC="images/menu/menu_left.gif" width=202 height=25 border=0></a></td>
    ...
    <td><IMG name ="spindle" SRC="images/menu/menu_spindle_on.gif" width=80 height=25 border=0></td>
   ...
   <td><table border=0 cellpadding=0 cellspacing=0 width=374>
     ...
     <tr><td rowspan=2><IMG SRC="images/menu/menu_bottom_search_left.gif" width=8 height=31 border=0></td>
       <td width=302><table border=0 cellpadding=0 cellspacing=0 width=366>
         <tr><td valign=bottom width=30><a href="spindle.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=27 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="spindle.asp?intRowStart=<%=intPreviousRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/previous_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="spindle.asp?intRowStart=<%=intNextRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/next_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=30><a href="spindle.asp?intRowStart=<%=intLastRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/last_button.gif" width=27 height=20 border=0></a></td>
...
<tr>
  ...
  <tr>
    ...
    <th class="itemlist" width=312><a href="spindle.asp?strOrderBy=SpindleType">Spindle Type</a></th>
    ...
    <th class="itemlist" width=80 align="center"><a href="spindle.asp?strOrderBy=Category">Category</a></th>
  </tr>  
  <!-- table data -->
  <%if( rsGetSpindles.State = adStateOpen ) then
				while( not rsGetSpindles.EOF )
					if( ( CInt( rsGetSpindles( "RowNumber" ) ) Mod 2 ) = 1 ) then%>
						<tr class="evenrow">
							...
							<td class="itemlist" width=312 <%if( strOrderBy = "SpindleType" ) then%> id="selected_evenrow"<%end if%>><%=rsGetSpindles( "SpindleType" )%></td>
					...
					<%else%>
						<tr class="oddrow">
							<td class="itemlist" width=40 align="center"><a href="javascript:OpenSpindle('<%=rsGetSpindles( "SpindleId" )%>');"><%=rsGetSpindles( "SpindleId" )%></a></td>
							<td class="itemlist" width=312 <%if( strOrderBy = "SpindleType" ) then%> id="selected_oddrow"<%end if%>><%=rsGetSpindles( "SpindleType" )%></td>
							<td class="itemlist" width=40 align="center"><%=rsGetSpindles( "Weight" )%></td>
							<td class="itemlist" width=40 align="center"><%=rsGetSpindles( "RPM" )%></td>
							<td class="itemlist" width=40 align="center"><%=rsGetSpindles( "Volts" )%></td>
							<td class="itemlist" width=40 align="center"><%=rsGetSpindles( "Hz" )%></td>
