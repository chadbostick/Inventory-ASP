...
<title>Centerline - Projects</title>
  ...
  function OpenProject( strProjectId )
	{
	  OpenWindow( 'project_details.asp?intProjectId='+strProjectId,'Centerline','width=657,height=540,left='+GetCenterX( 657 )+',top='+GetCenterY( 540 )+',screenX='+GetCenterX( 657 )+',screenY='+GetCenterY( 540 )+',scrollbars=yes,dependent=yes' );
	}
	...
	function JumpToSubmit()
	{
		...
		{
			document.location.href = "admin_project.asp?blnIsJumpTo=1&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if ( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "admin_project.asp?blnIsJumpTo=1&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	...
	function SearchSubmit()
	{
		if( document.all('strSearchTop').value != "" )
		{
			document.location.href = "admin_project.asp?blnIsJumpTo=0&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "admin_project.asp?blnIsJumpTo=0&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	
	function ConfirmDelete( intProjectId, strProject )
	{
		if( confirm( 'Are you sure you want to remove ' + strProject + '?' ) )
		{
			OpenWindow( 'project_delete.asp?intProjectId='+intProjectId,'DeleteRecord','width=657,height=120,left='+GetCenterX( 657 )+',top='+GetCenterY( 120 )+',screenX='+GetCenterX( 657 )+',screenY='+GetCenterY( 120 )+',scrollbars=yes,dependent=yes' );
		}
	}
   ...
   <td><table border=0 cellpadding=0 cellspacing=0 width=374>
     ...
     <tr><td rowspan=2><IMG SRC="images/menu/menu_bottom_search_left.gif" width=8 height=31 border=0></td>
       <td width=302><table border=0 cellpadding=0 cellspacing=0 width=366>
         <tr><td valign=bottom width=30><a href="admin_project.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=27 height=20 border=0></a></td>
				   <td valign=bottom width=27><a href="admin_project.asp?intRowStart=<%=intPreviousRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/previous_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="admin_project.asp?intRowStart=<%=intNextRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/next_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=30><a href="admin_project.asp?intRowStart=<%=intLastRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/last_button.gif" width=27 height=20 border=0></a></td>
...
<tr>
  ...
  <tr>
    ...
    <th class="itemlist"><a href="admin_project.asp?strOrderBy=ProjectName">Name</a></th>
    <th class="itemlist"><a href="admin_project.asp?strOrderBy=ProjectType">Project Type</a></th>
    <th class="itemlist"><a href="admin_project.asp?strOrderBy=Customer">Customer</a></th>
    <th class="itemlist">Sales Rep</th>
    <th class="itemlist"><a href="admin_project.asp?strOrderBy=StartDate">Start Date</a></th>
    <th class="itemlist"><a href="admin_project.asp?strOrderBy=EstCompletionDate">Est Complete</a></th>
    <th class="itemlist"><a href="admin_project.asp?strOrderBy=CompletionDate">Complete</a></th>
    <th class="itemlist"><a href="admin_project.asp?strOrderBy=ProjectDescription">Description</a></th>
  </tr>  
  <!-- table data -->
  <%if( rsGetProjects.State = adStateOpen ) then
				while( not rsGetProjects.EOF )
					if( ( CInt( rsGetProjects( "RowNumber" ) ) Mod 2 ) = 1 ) then%>
						<tr class="evenrow">
						  <td class="itemlist" width=20 align=middle><a href="javascript:ConfirmDelete('<%=rsGetProjects( "ProjectId" )%>', '<%=rsGetProjects( "ProjectName" )%>');"><img src="images/form_delete.gif" width=15 height=15 border=0></a></td>
						  <td class="itemlist" <%if( strOrderBy = "ProjectName" ) then%> id="selected_evenrow"<%end if%>><a href="javascript:OpenProject( '<%=rsGetProjects( "ProjectId" )%>' );"><%=rsGetProjects( "ProjectName" )%></a></td>
						  <td class="itemlist" <%if( strOrderBy = "ProjectType" ) then%> id="selected_evenrow"<%end if%>><%=rsGetProjects( "ProjectType" )%></td>
						  <td class="itemlist" <%if( strOrderBy = "Customer" ) then%> id="selected_evenrow"<%end if%>><%=rsGetProjects( "Customer" )%></td>
						  <td class="itemlist">Stephan</td>
						  <td class="itemlist" align=middle <%if( strOrderBy = "StartDate" ) then%> id="selected_evenrow"<%end if%>><%=rsGetProjects( "StartDate" )%></td>
						  <td class="itemlist" align=middle <%if( strOrderBy = "EstCompletionDate" ) then%> id="selected_evenrow"<%end if%>><%=rsGetProjects( "EstCompletionDate" )%></td>
						  <td class="itemlist" align=right <%if( strOrderBy = "CompletionDate" ) then%> id="selected_evenrow"<%end if%>><%=rsGetProjects( "CompletionDate" )%></td>
						  <td class="itemlist" <%if( strOrderBy = "ProjectDescription" ) then%> id="selected_evenrow"<%end if%>><%=rsGetProjects( "ProjectDescription" )%></td>
					<%else%>
						<tr class="oddrow">
							<td class="itemlist" width=20 align=middle><a href="javascript:ConfirmDelete('<%=rsGetProjects( "ProjectId" )%>', '<%=rsGetProjects( "ProjectName" )%>');"><img src="images/form_delete.gif" width=15 height=15 border=0></a></td>
						    <td class="itemlist" <%if( strOrderBy = "ProjectName" ) then%> id="selected_oddrow"<%end if%>><a href="javascript:OpenProject( '<%=rsGetProjects( "ProjectId" )%>' );"><%=rsGetProjects( "ProjectName" )%></a></td>
							<td class="itemlist" <%if( strOrderBy = "ProjectType" ) then%> id="selected_oddrow"<%end if%>><%=rsGetProjects( "ProjectType" )%></td>
							<td class="itemlist" <%if( strOrderBy = "Customer" ) then%> id="selected_oddrow"<%end if%>><%=rsGetProjects( "Customer" )%></td>
							<td class="itemlist">Stephan</td>
							<td class="itemlist" align=middle <%if( strOrderBy = "StartDate" ) then%> id="selected_oddrow"<%end if%>><%=rsGetProjects( "StartDate" )%></td>
							<td class="itemlist" align=middle <%if( strOrderBy = "EstCompletionDate" ) then%> id="selected_oddrow"<%end if%>><%=rsGetProjects( "EstCompletionDate" )%></td>
							<td class="itemlist" align=right <%if( strOrderBy = "CompletionDate" ) then%> id="selected_oddrow"<%end if%>><%=rsGetProjects( "CompletionDate" )%></td>
							<td class="itemlist" <%if( strOrderBy = "ProjectDescription" ) then%> id="selected_oddrow"<%end if%>><%=rsGetProjects( "ProjectDescription" )%></td>
					<%end if%>
						</tr>
