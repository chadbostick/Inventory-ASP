...
<title>Centerline - Quote</title>
  ...
  function OpenQuote( strQuoteId )
	{
	  OpenWindow( 'quote_details.asp?intQuoteId='+strQuoteId,'Centerline','width=657,height=480,left='+GetCenterX( 657 )+',top='+GetCenterY( 480 )+',screenX='+GetCenterX( 657 )+',screenY='+GetCenterY( 480 )+',scrollbars=yes,dependent=yes' );
	}
	...
	function JumpToSubmit()
	{
		...
		{
			document.location.href = "admin_quote.asp?blnIsJumpTo=1&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if ( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "admin_quote.asp?blnIsJumpTo=1&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	...
	function SearchSubmit()
	{
		if( document.all('strSearchTop').value != "" )
		{
			document.location.href = "admin_quote.asp?blnIsJumpTo=0&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "admin_quote.asp?blnIsJumpTo=0&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	
	function ConfirmDelete( intQuoteId, strQuote )
	{
		if( confirm( 'Are you sure you want to remove ' + strQuote + '?' ) )
		{
			OpenWindow( 'quote_delete.asp?intQuoteId='+intQuoteId,'DeleteRecord','width=657,height=120,left='+GetCenterX( 657 )+',top='+GetCenterY( 120 )+',screenX='+GetCenterX( 657 )+',screenY='+GetCenterY( 120 )+',scrollbars=yes,dependent=yes' );
		}
	}
   ...
   <td><table border=0 cellpadding=0 cellspacing=0 width=374>
     ...
     <tr><td rowspan=2><IMG SRC="images/menu/menu_bottom_search_left.gif" width=8 height=31 border=0></td>
       <td width=302><table border=0 cellpadding=0 cellspacing=0 width=366>
         <tr><td valign=bottom width=30><a href="admin_quote.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=27 height=20 border=0></a></td>
				   <td valign=bottom width=27><a href="admin_quote.asp?intRowStart=<%=intPreviousRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/previous_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="admin_quote.asp?intRowStart=<%=intNextRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/next_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=30><a href="admin_quote.asp?intRowStart=<%=intLastRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/last_button.gif" width=27 height=20 border=0></a></td>
...
<tr>
  ...
  <tr>
	...
	<th class="itemlist" width=40><a href="admin_quote.asp?strOrderBy=QuoteId">Quote Id</a></th>
    <th class="itemlist" width=40><a href="admin_quote.asp?strOrderBy=WorkOrderNumber">Work Order</a></th>
    <th class="itemlist"><a href="admin_quote.asp?strOrderBy=Customer">Customer</a></th>
    <th class="itemlist" width=140><a href="admin_quote.asp?strOrderBy=SpindleType">Spindle Type</a></th>
    <th class="itemlist" width=70><a href="admin_quote.asp?strOrderBy=DateQuoted">Date Quoted</a></th>
    <th class="itemlist" width=70>Date Approved</th>
    <th class="itemlist" width=80><a href="admin_quote.asp?strOrderBy=SerialNumber">Serial Number</a></th>
  </tr>
  <!-- table data -->
		<%if( rsGetQuotes.State = adStateOpen ) then
				while( not rsGetQuotes.EOF )
					if( ( CInt( rsGetQuotes( "RowNumber" ) ) Mod 2 ) = 1 ) then%>
						<tr class="evenrow">
							<td class="itemlist" width=20 align=middle><a href="javascript:ConfirmDelete('<%=rsGetQuotes( "QuoteId" )%>', '<%=rsGetQuotes( "QuoteId" )%>');"><img src="images/form_delete.gif" width=15 height=15 border=0></a></td>
							<td class="itemlist" align="center" <%if( strOrderBy = "QuoteId" ) then%> id="selected_evenrow"<%end if%>><a href="javascript:OpenQuote( '<%=rsGetQuotes( "QuoteId" )%>' );"><%=rsGetQuotes( "QuoteId" )%></a></td>
							<td class="itemlist" align="center" <%if( strOrderBy = "WorkOrderNumber" ) then%> id="selected_evenrow"<%end if%>><%=rsGetQuotes( "WorkOrderNumber" )%></td>
							<td class="itemlist" <%if( strOrderBy = "Customer" ) then%> id="selected_evenrow"<%end if%>><%=rsGetQuotes( "Customer" )%></td>
							<td class="itemlist" <%if( strOrderBy = "SpindleType" ) then%> id="selected_evenrow"<%end if%>><%=rsGetQuotes( "SpindleType" )%></td>
							<td class="itemlist" <%if( strOrderBy = "DateQuoted" ) then%> id="selected_evenrow"<%end if%>><%=rsGetQuotes( "DateQuoted" )%></td>
							<td class="itemlist"><%=rsGetQuotes( "DateApproved" )%></td>
							<td class="itemlist" <%if( strOrderBy = "SerialNumber" ) then%> id="selected_evenrow"<%end if%>><%=rsGetQuotes( "SerialNumber" )%></td>
					<%else%>
						<tr class="oddrow">
							<td class="itemlist" width=20 align=middle><a href="javascript:ConfirmDelete('<%=rsGetQuotes( "QuoteId" )%>', '<%=rsGetQuotes( "QuoteId" )%>');"><img src="images/form_delete.gif" width=15 height=15 border=0></a></td>
							<td class="itemlist" align="center" <%if( strOrderBy = "QuoteId" ) then%> id="selected_oddrow"<%end if%>><a href="javascript:OpenQuote( '<%=rsGetQuotes( "QuoteId" )%>' );"><%=rsGetQuotes( "QuoteId" )%></a></td>
							<td class="itemlist" align="center" <%if( strOrderBy = "WorkOrderNumber" ) then%> id="selected_oddrow"<%end if%>><%=rsGetQuotes( "WorkOrderNumber" )%></td>
							<td class="itemlist" <%if( strOrderBy = "Customer" ) then%> id="selected_oddrow"<%end if%>><%=rsGetQuotes( "Customer" )%></td>
							<td class="itemlist" <%if( strOrderBy = "SpindleType" ) then%> id="selected_oddrow"<%end if%>><%=rsGetQuotes( "SpindleType" )%></td>
							<td class="itemlist" <%if( strOrderBy = "DateQuoted" ) then%> id="selected_oddrow"<%end if%>><%=rsGetQuotes( "DateQuoted" )%></td>
							<td class="itemlist"><%=rsGetQuotes( "DateApproved" )%></td>
							<td class="itemlist" <%if( strOrderBy = "SerialNumber" ) then%> id="selected_oddrow"<%end if%>><%=rsGetQuotes( "SerialNumber" )%></td>
					<%end if%>
						</tr>
