...
<LINK REL=STYLESHEET TYPE="text/css" HREF="css/reports_JM.css" TITLE="tables_css">
        ...
        <%if( not rsReportOpenPurchaseOrders is nothing ) then
						if( rsReportOpenPurchaseOrders.State = adStateOpen ) then
							while( not rsReportOpenPurchaseOrders.EOF )%>
								<tr><td class="poid" align="center" valign="baseline"><a href="javascript:OpenPurchaseOrder( '<%=rsReportOpenPurchaseOrders( "PurchaseOrderId" )%>' );"><%=rsReportOpenPurchaseOrders( "PurchaseOrderId" )%></a></td>
