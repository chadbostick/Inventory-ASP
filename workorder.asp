...
if( ( intRowStart - intRowCount ) < 0 ) then
	intPreviousRecord = lngTotalWorkOrderCount - intRowCount
else
...
<title>Centerline - Work Order</title>
  ...
  function OpenWorkOrder( strWorkOrderId )
	...
	function JumpToSubmit()
	{
		...
		{
			document.location.href = "workorder.asp?blnIsJumpTo=1&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if ( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "workorder.asp?blnIsJumpTo=1&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	...
	function SearchSubmit()
	{
		if( document.all('strSearchTop').value != "" )
		{
			document.location.href = "workorder.asp?blnIsJumpTo=0&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "workorder.asp?blnIsJumpTo=0&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
...
<tr><td><table border=0 cellpadding=0 cellspacing=0 width=770>
  <tr><td><a href="index.html"><IMG SRC="images/menu/menu_left.gif" width=202 height=25 border=0></a></td>
    ...
    <td><IMG name ="work_order" SRC="images/menu/menu_work_order_on.gif" width=103 height=25 border=0></td>
   ...
   <td><table border=0 cellpadding=0 cellspacing=0 width=374>
     ...
     <tr><td rowspan=2><IMG SRC="images/menu/menu_bottom_search_left.gif" width=8 height=31 border=0></td>
       <td width=302><table border=0 cellpadding=0 cellspacing=0 width=366>
         <tr><td valign=bottom width=30><a href="workorder.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=27 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="workorder.asp?intRowStart=<%=intPreviousRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/previous_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="workorder.asp?intRowStart=<%=intNextRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/next_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=30><a href="workorder.asp?intRowStart=<%=intLastRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/last_button.gif" width=27 height=20 border=0></a></td>
...
<tr>
  ...
  <!-- table heading -->
  <tr>
    <th class="itemlist" width=40><a href="workorder.asp?strOrderBy=WorkOrder">Num</a></th>
    <th class="itemlist" width=90><a href="workorder.asp?strOrderBy=PONumber">PO #</a></th>
    <th class="itemlist" width=120><a href="workorder.asp?strOrderBy=Customer">Customer</a></th>
    <th class="itemlist" width=237><a href="workorder.asp?strOrderBy=SpindleType">Spindle</a></th>
    <th class="itemlist" width=25><a href="workorder.asp?strOrderBy=NewSpindle">New</a></th>
    <th class="itemlist" width=60><a href="workorder.asp?strOrderBy=DateIn">Date In</a></th>
    <th class="itemlist" width=60><a href="workorder.asp?strOrderBy=DateOut">Date Out</a></th>
    <th class="itemlist" width=50><a href="workorder.asp?strOrderBy=Priority">Priority</a></th>
  </tr>
  <!-- table data -->
		<%if( rsGetWorkOrders.State = adStateOpen ) then
				while( not rsGetWorkOrders.EOF )
					if( ( CInt( rsGetWorkOrders( "RowNumber" ) ) Mod 2 ) = 1 ) then%>
						<tr class="evenrow">
							...
							<td class="itemlist" width=227 <%if( strOrderBy = "SpindleType" ) then%> id="selected_evenrow"<%end if%>><%=rsGetWorkOrders( "SpindleType" )%></td>
					...
					<%else%>
						<tr class="oddrow">
							...
							<td class="itemlist" width=227 <%if( strOrderBy = "SpindleType" ) then%> id="selected_oddrow"<%end if%>><%=rsGetWorkOrders( "SpindleType" )%></td>
