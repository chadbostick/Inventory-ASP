...
dim rsGetProjectTypes, cmdGetProjectTypes
dim lngTotalProjectTypeCount, intRowStart, intRowCount, intNextRecord, intPreviousRecord, intFirstRecord, intLastRecord, strOrderBy, strSearchString, strJumpTo
dim strCurrentPage

set cnnSQLConnection		= nothing
set rsGetProjectTypes		= nothing
set cmdGetProjectTypes	= nothing

set cnnSQLConnection		= Server.CreateObject( "ADODB.Connection" )
set cmdGetProjectTypes	= Server.CreateObject( "ADODB.Command" )

cnnSQLConnection.Open Application( "ODBC_DSN" )
cnnSQLConnection.CursorLocation = adUseClient

cmdGetProjectTypes.ActiveConnection		= cnnSQLConnection
cmdGetProjectTypes.CommandText				= "{call spGetProjectTypes( ?,?,?,?,?,? ) }"

cmdGetProjectTypes.Parameters.Append cmdGetProjectTypes.CreateParameter( "@ErrorMessage", adVarChar, adParamOutput, 255 )
cmdGetProjectTypes.Parameters.Append cmdGetProjectTypes.CreateParameter( "@RowStart", adInteger, adParamInputOutput, 4 )
cmdGetProjectTypes.Parameters.Append cmdGetProjectTypes.CreateParameter( "@RowCount", adInteger, adParamInputOutput, 4 )
cmdGetProjectTypes.Parameters.Append cmdGetProjectTypes.CreateParameter( "@OrderBy", adVarChar, adParamInput, 64 )
cmdGetProjectTypes.Parameters.Append cmdGetProjectTypes.CreateParameter( "@SearchString", adVarChar, adParamInput, 255 )
cmdGetProjectTypes.Parameters.Append cmdGetProjectTypes.CreateParameter( "@JumpTo", adVarChar, adParamInput, 9 )
...
cmdGetProjectTypes.Parameters( "@RowStart" ).Value			= intRowStart
cmdGetProjectTypes.Parameters( "@RowCount" ).Value			= Request.QueryString( "intRowCount" )
cmdGetProjectTypes.Parameters( "@OrderBy" ).Value				= Request.QueryString( "strOrderBy" )
cmdGetProjectTypes.Parameters( "@SearchString" ).Value	= strSearchString
cmdGetProjectTypes.Parameters( "@JumpTo" ).Value				= strJumpTo

on error resume next
set rsGetProjectTypes = cmdGetProjectTypes.Execute
...
intRowCount = cmdGetProjectTypes.Parameters( "@RowCount" ).Value

if( rsGetProjectTypes.State = adStateOpen ) then
		lngTotalProjectTypeCount = rsGetProjectTypes( "TotalRowCount" )
		
		set rsGetProjectTypes = rsGetProjectTypes.NextRecordset
		
		if( rsGetProjectTypes.State = adStateOpen ) then
			if( not rsGetProjectTypes.EOF ) then
				intRowStart = rsGetProjectTypes( "RowNumber" )
			end if
		end if
end if

if( ( intRowStart + intRowCount ) > lngTotalProjectTypeCount ) then
	intNextRecord = 0
else
...
intFirstRecord = 1
intLastRecord = lngTotalProjectTypeCount - intRowCount + 1
...
<title>Centerline - Sub Work Sources</title>
  ...
  function OpenProjectType( strProjectTypeId )
	...
	function JumpToSubmit()
	{
		...
		{
			document.location.href = "projecttype.asp?blnIsJumpTo=1&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if ( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "projecttype.asp?blnIsJumpTo=1&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	...
	function SearchSubmit()
	{
		if( document.all('strSearchTop').value != "" )
		{
			document.location.href = "projecttype.asp?blnIsJumpTo=0&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "projecttype.asp?blnIsJumpTo=0&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	
	function ConfirmDelete( intProjectTypeId, strProjectType )
	{
		if( confirm( 'Are you sure you want to remove ' + strProjectType + '?' ) )
		{
			OpenWindow( 'projecttype_delete.asp?intProjectTypeId='+intProjectTypeId,'DeleteRecord','width=657,height=120,left='+GetCenterX( 657 )+',top='+GetCenterY( 120 )+',screenX='+GetCenterX( 657 )+',screenY='+GetCenterY( 120 )+',scrollbars=yes,dependent=yes' );
		}
	}
...
<tr><td><table border=0 cellpadding=0 cellspacing=0 width=770>
  <tr><td><a href="index.html"><IMG SRC="images/menu/menu_left.gif" width=202 height=25 border=0></a></td>
    <td><IMG name SRC="images/menu/menu_project_type_middle.gif" width=384 height=25 border=0></td>
   ...
   <td><table border=0 cellpadding=0 cellspacing=0 width=374>
     ...
     <tr><td rowspan=2><IMG SRC="images/menu/menu_bottom_search_left.gif" width=8 height=31 border=0></td>
       <td width=302><table border=0 cellpadding=0 cellspacing=0 width=366>
         <tr><td valign=bottom width=30><a href="projecttype.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=27 height=20 border=0></a></td>
				   <td valign=bottom width=27><a href="projecttype.asp?intRowStart=<%=intPreviousRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/previous_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="projecttype.asp?intRowStart=<%=intNextRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/next_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=30><a href="projecttype.asp?intRowStart=<%=intLastRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/last_button.gif" width=27 height=20 border=0></a></td>
...
<tr>
  ...
  <tr>
    ...
    <th class="itemlist"><a href="projecttype.asp?strOrderBy=ProjectType">Project Type</a></th>
  </tr>  
  <!-- table data -->
	<%if( rsGetProjectTypes.State = adStateOpen ) then
			while( not rsGetProjectTypes.EOF )
				if( ( CInt( rsGetProjectTypes( "RowNumber" ) ) Mod 2 ) = 1 ) then%>
					<tr class="evenrow">
				<%else%>
					<tr class="oddrow">
				<%end if%>
						<td class="itemlist" width=20 align=middle><a href="javascript:ConfirmDelete('<%=rsGetProjectTypes( "ProjectTypeId" )%>', '<%=rsGetProjectTypes( "ProjectType" )%>');"><img src="images/form_delete.gif" width=15 height=15 border=0></a></td>
						<td class="itemlist"><a href="javascript:OpenProjectType('<%=rsGetProjectTypes( "ProjectTypeId" )%>');"><%=rsGetProjectTypes( "ProjectType" )%></a></td>
				</tr>
			<%rsGetProjectTypes.MoveNext
		wend
	end if%>
</table>
...
<tr><td><table cellspacing=0 cellpadding=0 width=672 align=right>
  ...
  <tr><td valign=bottom width=30><a href="projecttype.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=Request.QueryString( "strOrderBy" )%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=27 height=20 border=0></a></td>
    ...
    <td align=right valign=bottom width=236><a href="javascript:OpenProjectType( 0 );"><IMG src="images/add_button.gif" width=60 height=20 border=0></a></td>
...
<%
if not( rsGetProjectTypes is nothing ) then
	if( rsGetProjectTypes.State = adStateOpen ) then
		rsGetProjectTypes.Close
	end if
end if
