...
dim lngTotalWorkOrderCount, intRowStart, intRowCount, intFirstRecord, intLastRecord, intNextRecord, intPreviousRecord, strOrderBy, strSearchString, strJumpTo
dim strCurrentPage
...
<title>Centerline - Work Order</title>
  ...
  function OpenWorkOrder( strWorkOrderId )
	...
	function JumpToSubmit()
	{
		...
		{
			document.location.href = "workorder_search.asp?blnIsJumpTo=1&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if ( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "workorder_search.asp?blnIsJumpTo=1&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	...
	function SearchSubmit()
	{
		if( document.all('strSearchTop').value != "" )
		{
			document.location.href = "workorder_search.asp?blnIsJumpTo=0&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "workorder_search.asp?blnIsJumpTo=0&strOrderBy=<%=strOrderBy%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	...
	{
		window.opener.SelectWorkOrder( intWorkOrderId, strWorkOrderNumber);
		window.close();
	}
	</script>
</head>
   ...
   <td><table border=0 cellpadding=0 cellspacing=0 width=374>
     ...
     <tr><td rowspan=2><IMG SRC="images/menu/menu_bottom_search_left.gif" width=8 height=31 border=0></td>
       <td width=302><table border=0 cellpadding=0 cellspacing=0 width=366>
         <tr><td valign=bottom width=27><a href="workorder_search.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="workorder_search.asp?intRowStart=<%=intPreviousRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/previous_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="workorder_search.asp?intRowStart=<%=intNextRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/next_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="workorder_search.asp?intRowStart=<%=intLastRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/last_button.gif" width=24 height=20 border=0></a></td>
...
<tr>
  ...
  <tr>
    ...
    <th class="itemlist" width=40><a href="workorder_search.asp?strOrderBy=WorkOrder">Num</a></th>
    <th class="itemlist" width=90><a href="workorder_search.asp?strOrderBy=PONumber">PO #</a></th>
    <th class="itemlist" width=120><a href="workorder_search.asp?strOrderBy=Customer">Customer</a></th>
    <th class="itemlist" width=187><a href="workorder_search.asp?strOrderBy=SpindleType">Spindle</a></th>
    <th class="itemlist" width=25><a href="workorder_search.asp?strOrderBy=NewSpindle">New</a></th>
    <th class="itemlist" width=60><a href="workorder_search.asp?strOrderBy=DateIn">Date In</a></th>
    <th class="itemlist" width=60><a href="workorder_search.asp?strOrderBy=DateOut">Date Out</a></th>
    <th class="itemlist" width=50><a href="workorder_search.asp?strOrderBy=Priority">Priority</a></th>
  </tr>
  <!-- table data -->
		<%if( rsGetWorkOrders.State = adStateOpen ) then
				while( not rsGetWorkOrders.EOF )
					if( ( CInt( rsGetWorkOrders( "RowNumber" ) ) Mod 2 ) = 1 ) then%>
						<tr class="evenrow">
							<td class="itemlist" width=20 align=middle><a href="javascript:SelectWorkOrder('<%=rsGetWorkOrders( "WorkOrderId" )%>', '<%=rsGetWorkOrders( "WorkOrderNumber" )%>');"><img src="images/form_select.gif" width=15 height=15 border=0></a></td>
						  	<td class="itemlist" width=40 align="center"<%if( strOrderBy = "WorkOrder" ) then%> id="selected_evenrow"<%end if%>><a href="javascript:OpenWorkOrder( '<%=rsGetWorkOrders( "WorkOrderId" )%>' );"><%=rsGetWorkOrders( "WorkOrderNumber" )%></a></td>
							<td class="itemlist" width=90 <%if( strOrderBy = "PONumber" ) then%> id="selected_evenrow"<%end if%>><%=rsGetWorkOrders( "PONumber" )%></td>
							<td class="itemlist" width=120 <%if( strOrderBy = "Customer" ) then%> id="selected_evenrow"<%end if%>><%=rsGetWorkOrders( "Customer" )%></td>
							<td class="itemlist" width=187 <%if( strOrderBy = "SpindleType" ) then%> id="selected_evenrow"<%end if%>><%=rsGetWorkOrders( "SpindleType" )%></td>
					...
					<%else%>
						<tr class="oddrow">
							<td class="itemlist" width=20 align=middle><a href="javascript:SelectWorkOrder('<%=rsGetWorkOrders( "WorkOrderId" )%>', '<%=rsGetWorkOrders( "WorkOrderNumber" )%>');"><img src="images/form_select.gif" width=15 height=15 border=0></a></td>
							<td class="itemlist" width=40 align="center"<%if( strOrderBy = "WorkOrder" ) then%> id="selected_oddrow"<%end if%>><a href="javascript:OpenWorkOrder( '<%=rsGetWorkOrders( "WorkOrderId" )%>' );"><%=rsGetWorkOrders( "WorkOrderNumber" )%></a></td>
							<td class="itemlist" width=90 <%if( strOrderBy = "PONumber" ) then%> id="selected_oddrow"<%end if%>><%=rsGetWorkOrders( "PONumber" )%></td>
							<td class="itemlist" width=120 <%if( strOrderBy = "Customer" ) then%> id="selected_oddrow"<%end if%>><%=rsGetWorkOrders( "Customer" )%></td>
							<td class="itemlist" width=187 <%if( strOrderBy = "SpindleType" ) then%> id="selected_oddrow"<%end if%>><%=rsGetWorkOrders( "SpindleType" )%></td>
...
<tr><td><table cellspacing=0 cellpadding=0 width=672 align=right>
  ...
  <tr><td valign=bottom width=27><a href="workorder_search.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=24 height=20 border=0></a></td>
    ...
    <td align=right valign=bottom width=260><a href="javascript:OpenWorkOrder( 0 );"><IMG src="images/add_button.gif" width=60 height=20 border=0></a></td>
