...
dim lngTotalSupplierCount, intRowStart, intRowCount, intFirstRecord, intLastRecord, intNextRecord, intPreviousRecord, strOrderBy, strSearchString, strJumpTo
dim strCurrentPage
...
<title>Centerline - Suppliers</title>
  ...
  function OpenSupplier( strSupplierId )
	...
	function JumpToSubmit()
	{
		...
		{
			document.location.href = "supplier_search.asp?blnIsJumpTo=1&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if ( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "supplier_search.asp?blnIsJumpTo=1&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	...
	function SearchSubmit()
	{
		if( document.all('strSearchTop').value != "" )
		{
			document.location.href = "supplier_search.asp?blnIsJumpTo=0&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchTop').value;
		}
		else if( document.all('strSearchBottom').value != "" )
		{
			document.location.href = "supplier_search.asp?blnIsJumpTo=0&strOrderBy=<%=Request.QueryString( "strOrderBy" )%>&strSearch=" + document.all('strSearchBottom').value;
		}
	}
	...
	{
		window.opener.SelectSupplier( intSupplierId, strSupplierName);
		window.close();
	}
	</script>
</head>
   ...
   <td><table border=0 cellpadding=0 cellspacing=0 width=374>
     ...
     <tr><td rowspan=2><IMG SRC="images/menu/menu_bottom_search_left.gif" width=8 height=31 border=0></td>
       <td width=302><table border=0 cellpadding=0 cellspacing=0 width=366>
         <tr><td valign=bottom width=27><a href="supplier_search.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="supplier_search.asp?intRowStart=<%=intPreviousRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/previous_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="supplier_search.asp?intRowStart=<%=intNextRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/next_button.gif" width=24 height=20 border=0></a></td>
           <td valign=bottom width=27><a href="supplier_search.asp?intRowStart=<%=intLastRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/last_button.gif" width=24 height=20 border=0></a></td>
...
<tr>
  ...
  <tr>
    ...
    <th class="itemlist"><a href="supplier_search.asp?strOrderBy=SupplierName">Supplier</a></th>
    <th class="itemlist"><a href="supplier_search.asp?strOrderBy=ContactName">Contact</a></th>
    <th class="itemlist"><a href="supplier_search.asp?strOrderBy=ContactTitle">Title</a></th>
  ...
  <%
	if( rsGetSuppliers.State = adStateOpen ) then
		while( not rsGetSuppliers.EOF )
			...
			<tr class="oddrow">
			<%end if%>
			<td class="itemlist" width=20 align=middle><a href="javascript:SelectSupplier('<%=rsGetSuppliers( "SupplierId" )%>', '<%=rsGetSuppliers( "SupplierName" )%>' );"><img src="images/form_select.gif" width=15 height=15 border=0></a></td>
...
<tr><td><table cellspacing=0 cellpadding=0 width=672 align=right>
  ...
  <tr><td valign=bottom width=27><a href="supplier_search.asp?intRowStart=<%=intFirstRecord%>&strOrderBy=<%=strOrderBy%><%if( strSearchString <> "" ) then%>&strSearch=<%Response.Write( strSearchString )%><%end if%>"><IMG src="images/search_bar/first_button.gif" width=24 height=20 border=0></a></td>
    ...
    <td align=right valign=bottom width=296><a href="javascript:OpenSupplier( 0 );"><IMG src="images/add_button.gif" width=60 height=20 border=0></a></td>
